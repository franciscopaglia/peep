// The 48 Shavian letters — the single source of truth for glyph reference data
// (the About page table, the Landing chart, and letter-intro teach cards).
export type ShavianLetter = {
  glyph: string;
  sound: string;
  key: string;
  ipa: string;
...
export const SHAVIAN_ALPHABET: ShavianLetter[] = [
...
const BY_GLYPH = new Map(SHAVIAN_ALPHABET.map((l) => [l.glyph, l]));

/** Letters for the given glyphs, in the order asked for; unknown glyphs are dropped. */
export function lettersFor(glyphs: string[]): ShavianLetter[] {
  return glyphs.map((g) => BY_GLYPH.get(g)).filter((l): l is ShavianLetter => l !== undefined);
}
